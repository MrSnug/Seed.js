...
// No need to require SeedTrackerBasic here, since loadPlugins will do it for you.
...
async function main() {
  try {
    ...
    // 3) Perform startup checks → returns the Discord client
    const discordClient = await performStartupChecks(config);

    // 3.5) Optionally reload Discord slash commands
    ...
    // ──> Remove the manual loadPlugin call. loadPlugins() will pick up SeedTrackerBasic.js automatically.
    // serverInstance.loadPlugin(new SeedTrackerBasic(config));

    // 5) Load all plugins (including SeedTrackerBasic) based on config.plugins
    const loadedPlugins = await loadPlugins(config);

    // 6) Mount each plugin with both serverInstance & Discord client
    await mountPlugins(loadedPlugins, serverInstance, discordClient);

    // 7) Set up CommandHandler for any other slash commands
    ...
    // Listen for incoming slash‐command interactions
    ...
    // 8) Start the “players” loop (RCON or however your main server sends player arrays)
